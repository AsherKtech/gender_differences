...
# Dynamically detect country and round column names in df_raw
cntry_col = next((c for c in ['cntry', 'Country', 'country'] if c in df_raw.columns), None)
round_col = next((c for c in ['ESS_round', 'round', 'Round', 'essround'] if c in df_raw.columns), None)

if not cntry_col or not round_col:
    raise KeyError(f"Could not find country/round columns in df_raw. Found columns: {list(df_raw.columns[:10])}...")

# 1. Create a complete MultiIndex of all unique (Country, Round) combinations present in df_raw
all_country_rounds = pd.MultiIndex.from_frame(
    df_raw[[cntry_col, round_col]].drop_duplicates(),
    names=['Country', 'ESS_round']
)

# 2. Compute non-missing response counts per column
counts_list = []
for col in names:
    if col in df_raw.columns:
        # Count non-null observations grouped by country and round
        col_counts = (
            df_raw[df_raw[col].notna()]
            .groupby([cntry_col, round_col])
            .size()
            .rename(col)
        )
    else:
        # If column is completely missing from df_raw, create empty Series with right index names
        col_counts = pd.Series(dtype=int, name=col)
        col_counts.index.names = [cntry_col, round_col]

    # Reindex against all_country_rounds to explicitly fill missing combos with 0
    col_counts = col_counts.reindex(all_country_rounds, fill_value=0)
    counts_list.append(col_counts)

# 3. Concatenate all feature columns into a unified table
missingness_table = pd.concat(counts_list, axis=1).reset_index()

# Print sample
print("\nMissingness summary table (non-missing counts per column, country, round):")
pd.set_option('display.max_rows', None)
pd.set_option('display.width', 200)
print(missingness_table.head(15).to_string(index=False))

# Save to CSV
missingness_table.to_csv(here('data/processed/columns_missingness_by_country_round.csv'), index=False)
print(f"\n✅ Missingness summary saved to 'data/processed/columns_missingness_by_country_round.csv'.")


#%%
# Convert missingness_table from wide to long format to identify 0-count entries
zero_counts = missingness_table.melt(
    id_vars=['Country', 'ESS_round'],
    value_vars=names,
    var_name='variable',
    value_name='count'
)

# Filter for combinations where count is 0
zero_summary = zero_counts[zero_counts['count'] == 0].copy()

# Sort and clean up output
zero_summary = zero_summary.sort_values(['variable', 'Country', 'ESS_round']).drop(columns=['count']).reset_index(drop=True)

# Display the summary table
print("\n=== SUMMARY OF MISSING VARIABLES (Count = 0) ===")
if zero_summary.empty:
    print("All variables have valid responses across all country-round combinations!")
else:
    pd.set_option('display.max_rows', None)
    print(zero_summary.to_string(index=False))

# Grouped summary: list countries and rounds for each missing variable
if not zero_summary.empty:
    grouped_series = zero_summary.groupby('variable').apply(
        lambda g: g.groupby('Country')['ESS_round'].apply(list).to_dict(),
        include_groups=False
    )
    grouped_zero_summary = grouped_series.reset_index(name='missing_in_country_rounds')

    print("\n=== COMPACT SUMMARY BY VARIABLE ===")
    for _, row in grouped_zero_summary.iterrows():
        print(f"\nVariable: {row['variable']}")
        for country, rounds in row['missing_in_country_rounds'].items():
            print(f"  - {country}: Rounds {rounds}")
else:
    print("\n=== COMPACT SUMMARY BY VARIABLE ===")
    print("No missing variables to display.")

# Save zero counts summary to CSV
zero_summary.to_csv(here('data/processed/variables_with_zero_responses.csv'), index=False)
print(f"\n✅ Zero-response summary saved to 'data/processed/variables_with_zero_responses.csv'.")
