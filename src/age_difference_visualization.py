...
def create_unified_age_countries_plot(perf_df, output_html="countries_age_similarity_trends_unified.html", min_rounds=1, country_map=None):
    """
    ...
    preserving underscores and specific suffix tags for Age Model results.
    """
    ...
    base_prefix = "countries_age_similarity_trends_"
    
    # Words/tokens to ignore if present
    ignored_words = {
        "countries", "age", "similarity", 
        "trends", "predictability", "accuracy", "plot", "unified"
    }
    ...
    title_text = f"<b>Age Predictability Trends{suffix_label}</b><br><sup>Lower Accuracy = Higher Inter-Generational Similarity</sup>"
...
perf_df = pd.read_csv(here("data/processed/Age_country_and_round_performance_indicators.csv"))
...
create_unified_age_countries_plot(
    perf_df,
    min_rounds=6, 
    output_html=here("plots/countries_age_similarity_trends_nw_europe.html"), 
...
create_unified_age_countries_plot(
    perf_df, 
    min_rounds=6, 
    output_html=here("plots/countries_age_similarity_trends_other.html"),  
